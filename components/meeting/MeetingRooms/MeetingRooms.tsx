...
import { Plus, Calendar as CalendarIcon, Building } from "lucide-react";
...
const MeetingRooms: React.FC = () => {
  ...
  // Fetch rooms
  const { data: roomsData, isLoading: roomLoading } = useQuery({
  ...
  return (
    <MeetingRoomsContainer>
      ...
      {roomLoading ? (
        ...
        <NoRooms>
          <Building size={48} />
          <NoRoomsMessage>Chưa có phòng họp nào được thiết lập</NoRoomsMessage>
        </NoRooms>
      ) : (
        <MeetingRoomsContent>
          <CalendarSection>
              ...
              {isLoadingMeetings ? (
                ...
                <MeetingList>
                  {myMeetings.map((meeting) => {
                    return (
                      <MeetingItem key={meeting.id} $isMyMeeting={true}>
                        <MeetingItemHeader>
                          <h4>{meeting.title}</h4>
