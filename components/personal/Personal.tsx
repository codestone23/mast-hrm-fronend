...
const Personal: React.FC = () => {
	...
	// Fetch my meetings today and tomorrow
	const today = new Date();
	today.setHours(0, 0, 0, 0);
	const tomorrow = new Date(today);
	tomorrow.setDate(tomorrow.getDate() + 1);

	const { data: myMeetingsData, isLoading: isLoadingMeetings } = useQuery({
		queryKey: [
			"my-meetings",
					...
					<DashboardCol>
						<NewsSidebar>
							<NewsSidebarHeader>
								...
								<NewsSidebarViewMore
									onClick={() =>
										router.push(`${ROUTERS.PERSONAL.COMPANY}?tab=news`)
									}
								>
									Xem thêm
									<ArrowRight
