...
const LateArrivalModal: React.FC<LateArrivalModalProps> = ({
  isOpen,
  onClose,
  selectedDate
}) => {
  const [formData, setFormData] = useState({
    proposalName: 'Đi muộn vì hỏng xe',
    approver: '',
    applicationDate: selectedDate,
    lateTime: '17:00',
    registeredMinutes: '25'
  });
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      showSuccessToast('Đăng ký đi muộn thành công!');
      onClose();
    } catch (error) {
      console.error('Error submitting late arrival request:', error);
        ...
        <FormSection>
          <FormGrid>
            ...
            <TimePicker
              ...
              onChange={(value) => handleInputChange('lateTime', value)}
              required
              disabled={isLoading}
            />
            
            <Input
              label="Số phút đi muộn được đăng ký"
