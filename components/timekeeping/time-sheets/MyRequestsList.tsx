...
import { FileText, Calendar, Filter, Edit, Trash2 } from "lucide-react";
import {
  Select,
  DatePicker,
  Pagination,
...
const MyRequestsList: React.FC<MyRequestsListProps> = ({
  ...
  const handleDeleteClick = (e: React.MouseEvent, request: Request) => {
    e.stopPropagation();
    const endpointType = getRequestTypeEndpoint(
  ...
  return (
    <ListRequestContainer>
      <ListRequestHeader>
        <div>
          <ListRequestTitle>Danh sách yêu cầu của tôi</ListRequestTitle>
          <ListRequestSubtitle>
            Tổng cộng{" "}
            <ListRequestHighlight>
              {pagination?.total || 0}
        ...
        <>
          {requests.length === 0 ? (
            <EmptyStateContainer>
              ...
              <EmptyStateDescription>
                Bạn chưa tạo yêu cầu nào hoặc không có yêu cầu nào phù hợp với
                bộ lọc hiện tại.
              </EmptyStateDescription>
            </EmptyStateContainer>
          ) : (
                ...
                {requests.map((request, index) => (
                  ...
                  >
                    <RequestHeader>
                      <div>
                        <RequestTitle>
                          Loại: {getTypeLabel(request.type as REQUEST_TYPE)}
                        </RequestTitle>
                        <RequestNote>Tiêu đề: {request.title}</RequestNote>
                        <RequestDescription>
                      ...
                      <RequestRight>
                        ...
                        <RequestMeta>
                          ...
                          <RequestMetaItem>
                            <span>•</span>
                            <span>
                              Ngày gửi: {formatDateTime(request.created_at)}
                            </span>
                          </RequestMetaItem>
                        </RequestMeta>
                      </RequestRight>
